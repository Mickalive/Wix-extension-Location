...
// The official scaffold owns dependency placement. A package present in the
// generated runtime dependencies must not remain duplicated in devDependencies.
for (const name of Object.keys(merged.dependencies || {})) {
  if (merged.devDependencies && Object.prototype.hasOwnProperty.call(merged.devDependencies, name)) {
    delete merged.devDependencies[name];
  }
}
...
# `.astro/` is generated by Astro/Wix builds. It must never become immutable
# product source or a factory candidate; each build regenerates it locally.
touch "$PRODUCT/.gitignore"
if ! grep -qxF '.astro/' "$PRODUCT/.gitignore"; then
  printf '\n# Astro/Wix generated state\n.astro/\n' >>"$PRODUCT/.gitignore"
fi
rm -rf "$PRODUCT/.astro"
