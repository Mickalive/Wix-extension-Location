...
if [[ "$STATUS" == active ]]; then
  ...
  bash .github/scripts/run-opencode-with-retry.sh opencode run --model "$OX_MODEL" --agent "$AGENT" \
    "Run Wix product lane $ROLE, cycle $CYCLE_INDEX, from immutable accepted SHA $BASE_SHA. Read your immutable role fiche first, then MAIN_PROMPT.md, AGENTS.md, docs/WIX_TECHNICAL_CONTRACT.md, docs/BUILD_BLUEPRINT.md, $DIRECTIVE, docs/NEXT_CYCLE.json and latest persisted evidence. Exact task: $task. Why: $why. Evidence: $evidence. Criteria: $criteria. Repair current negative-audit blockers before unrelated work. Otherwise do ONLY this task. Do not alter orchestration, governance, job descriptions, other lanes, or commit/push. Never request/read/print secrets. Human note: ${HUMAN_NOTE:-}"
  ...
  while IFS= read -r -d '' path; do
    ...
    case "$path" in
      .env|.env.*|*/.env|*/.env.*|*.key|*.pem|*.p8|*.p12|*credentials*|.wix/*|.wix/**)
        echo "::error::Credential-like file forbidden: $path"; unexpected=1 ;;
    esac
