...
If the global decision is `continue`, `docs/NEXT_CYCLE.json` must contain at least one `active` lane and every active lane must have a non-empty exact task, why it is needed, source evidence and measurable acceptance criteria. Any `repair_lanes` entry must be active and must address the latest persisted audit blockers before new work.

If no evidence-backed useful work remains and deterministic gates pass, choose `release_candidate`; do NOT manufacture another cycle.

Write both `reports/director/CYCLE_<run>.md` and `reports/director/CYCLE_<run>.json` with accepted/rejected changes, audit responses, tests run, residual risks, repair lanes and one decision: `continue`, `stop`, or `release_candidate`.
